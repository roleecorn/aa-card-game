...
describe('additional discussion-ranked character cards', () => {
  it('adds the six requested characters with current calibrated values', () => {
    expect(Object.keys(CHARACTERS)).toEqual(expect.arrayContaining(['lemon', 'meteor', 'emotion', 'yashiro', 'avocado', 'kitsu']));

    expect(CHARACTERS.meteor?.stats).toEqual({ design: 0, text: 1, aa: 2 });
    expect(CHARACTERS.meteor?.maxStress).toBe(4);
    expect(SKILLS.meteorBurnDesign?.status).toBe('implemented');
    expect(SKILLS.meteorResonance?.status).toBe('implemented');

    expect(CHARACTERS.yashiro?.stats).toEqual({ design: 0, text: 0, aa: 3 });
...
describe('流星 complete character package', () => {
  ...
  it('uses the latest discussion-backed stats and skills', () => {
    expect(CHARACTERS.meteor?.stats).toEqual({ design: 0, text: 1, aa: 2 });
    expect(CHARACTERS.meteor?.maxStress).toBe(4);
...
describe('八代 complete character package', () => {
  ...
  it('uses the latest sourced stats and production portrait', () => {
    expect(CHARACTERS.yashiro?.stats).toEqual({ design: 0, text: 0, aa: 3 });
    expect(CHARACTERS.yashiro?.maxStress).toBe(5);
  ...
  it('可愛 reduces the highest allied stress and 好學 adds two Guide cards at round start', () => {
...
describe('情緒 complete character package', () => {
  ...
  it('uses the calibrated AA value and current triggered skill', () => {
    expect(CHARACTERS.emotion?.stats).toEqual({ design: 1, text: 1, aa: 2 });
    expect(CHARACTERS.emotion?.maxStress).toBe(5);
    expect(CHARACTERS.emotion?.portrait).toBe('/assets/characters/portrait/emotion.webp');
    expect(CHARACTERS.emotion?.skillIds).toEqual(['emotionSpinningTop']);
    expect(SKILLS.emotionSpinningTop?.status).toBe('implemented');
  });

  it('屬陀螺的 reduces leader Stress the first time 情緒 is targeted by a coordination card each round', () => {
    const game = createInitialGame(fixedRng(0.5), STANDARD_GAME_DEFINITION, EMOTION_ROSTER);
    const engine = new EngineSession(game, fixedRng(0.5));
    const leader = engine.getCharacter('player', game.player.leaderId)!;
    leader.stress = 1;
    engine.addCard('player', 'guide', 2);
    const cards = game.player.hand.filter((card) => card.cardId === 'guide').slice(-2);
...
describe('generic effect vocabulary', () => {
  it('can choose members by stress without character-specific code', () => {
    ...
    expect(engine.applyEffects([{ kind: 'stress.change', target: 'highestStressEnemy', amount: 1, source: 'test' }], context())).toBe(true);
  ...
  it('supports selected-die modification, conversion, and filtered removal', () => {
    ...
    expect(engine.applyEffects([{ kind: 'dice.modifySelected', add: 2 }], { ...context('mashiro'), activationTarget: { targetDieId: dice[0]!.id } })).toBe(true);
    expect(dice[0]!.value).toBe(3);
    expect(engine.applyEffects([{ kind: 'dice.convertPending', target: 'owner', fromSkill: 'design', toSkill: 'text', count: 1 }], context('mashiro'))).toBe(true);
    expect(dice.filter((die) => die.skill === 'text')).toHaveLength(1);
    expect(engine.applyEffects([{ kind: 'dice.removePending', target: 'owner', count: 1, order: 'highest' }], context('mashiro'))).toBe(true);
  ...
  it('can clear existing progress generically', () => {
    ...
    expect(engine.applyEffects([{ kind: 'work.progress.clear', target: 'selectedWork', count: 2, order: 'lowest' }], { ...context(), activationTarget: { workId: work.id } })).toBe(true);
...
describe('discussion-backed character catalog', () => {
  it('includes every character card with explicit values in the discussion notes', () => {
    expect(Object.keys(CHARACTERS)).toEqual(expect.arrayContaining(['pintbox', 'user79', 'mashiro', 'ginsakura', 'narrator', 'bluewind', 'triangle', 'fengyang', 'happy', 'chaos']));
...
describe('random standard roster selection', () => {
  it('draws three player characters, then three opponents from the remaining pool', () => {
    ...
    expect(selected.unusedMemberIds).toHaveLength(Object.values(CHARACTERS).filter((character) => isStandardPlayableCharacterId(character.id)).length - 6);
  });
...
describe('高興 complete character package', () => {
  ...
  it('the generic random-card effect only adds cards of the requested kind', () => {
    ...
    expect(engine.applyEffects([{ kind: 'custom', handler: 'addRandomCardsByKind', args: { cardKind: 'coordination', count: 3 } }], context())).toBe(true);
...
describe('卡奧斯 complete character package', () => {
  function createChaosGame(rng: () => number = fixedRng(0.5)): { game: GameState; engine: EngineSession } {
    ...
    const game: GameState = {
      ...
      phase: 'player-plan',
      player: {
        id: 'player', name: 'player', leaderId: 'chaos',
        members: [{ defId: 'chaos', stress: 0, permanentStats: { ...definition.stats }, timedStatModifiers: [], skillUsage: {}, statuses: {}, resources: resource ? { [resource.name]: resource.initial } : undefined }],
        works: [], hand: [], deck: [], discard: [], pendingDice: [],
      },
      enemy: { id: 'enemy', name: 'enemy', leaderId: '', members: [], works: [], hand: [], deck: [], discard: [], pendingDice: [] },
      logs: [],
    };
