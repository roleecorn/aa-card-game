...
export const akikageCharacter = characterDefinitionSchema.parse({
  id: 'akikage', name: '秋影', stats: { design: 1, text: 2, aa: 1 }, maxStress: 3, affinities: ['情'],
  skillIds: ['akikageDeadlineWarrior', 'akikageProcrastination'], portrait: 'assets/characters/portrait/akikage.webp', compactPortrait: 'assets/characters/compact/akikage.webp',
  sourceNotes: ['2026-09-10：卡奧斯提出原數值與技能，PintBox重新確認。', '2026-09-11：卡奧斯提出秋影適性（情），PintBox以👌認可。'],
});
export const akikageSkills = skillDefinitionSchema.array().parse([
  ...
  { id: 'akikageProcrastination', name: '拖延症', description: '自己擲出的 1、2 不能使用。', activation: 'triggered', status: 'implemented', triggers: [
    ...
    { event: 'afterDiceGranted', condition: { kind: 'all', conditions: [{ kind: 'relation', field: 'targetId', relation: 'self' }, { kind: 'diceMatch', maxValue: 2 }] }, effects: [{ kind: 'custom', handler: 'removeEventDiceAtOrBelow', args: { maxValue: 2 } }] },
  ] },
]);
