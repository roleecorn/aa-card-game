...
export function createTutorialGame(): GameState {
  ...
  // The tutorial demonstrates Grimm's active skill, whose current rule requires
  // his own work to be an emotion work. Keep the fixture explicit instead of
  // depending on affinity randomization.
  const grimmWork = game.player.works.find((work) => work.ownerId === 'grimm');
  if (grimmWork) grimmWork.type = '情';
